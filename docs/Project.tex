...
\maketitle
\small{
GitHub repository:\\ github.com/chysch/Phonetic-Pun-ishment}
...
Since puns don't always cover all the words of the sentence (they usually don't), we added functionality to control the amount of words translated for shorter running time in the next stages. 
...
The 'Logon' tool \citepalias{the_logon_project} is used to parse a sentence to its derivation tree. This tool helps us to indicate whether a sentence is syntactically valid. In the case that it doesn't, it retrieves zero parsing trees for the sentence.
...
The rule file the user is required to provide can be empty, but usually will contain values for the hyper-parameters controlling the project. Below the hyper-parameters are listed by name with a short description and motivation.
